...
public class TechFoodPageProcessor implements PageProcessor {
	...
	public void process(Page page) {
		...
		while (iter.hasNext()) {
			iter.set("http://www.tech-food.com/news" + iter.next());
		}

		page.addTargetRequests(list);
		*/
		
		page.addTargetRequests(page.getHtml().xpath("//div[@class='news_content3']//a/@href").all());
		...
		foodMateRepo.setTime(page.getHtml()
				.xpath("//div[@class='biaoti1x']/text()").regex("(\\d{2}|\\d{4})(?:\\-)?([0]{1}\\d{1}|[1]{1}[0-2]{1})(?:\\-)?([0-2]{1}\\d{1}|[3]{1}[0-1]{1})(?:\\s)?([0-1]{1}\\d{1}|[2]{1}[0-3]{1})(?::)?([0-5]{1}\\d{1})(?::)?([0-5]{1}\\d{1})").toString());
